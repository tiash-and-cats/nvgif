...
## 2. Compression Flags
The `Compression` byte is a bitmask of `CompressionType`:
...
Multiple flags may be combined (e.g. `RLE | ZLIB`).
...
- **Alpha support**: only available in raw RGB/RGBA modes.
...
```
Header: NVG 05 03 01 00 02 02 00 Payload: [zlib-compressed RLE stream of row data]
```