...
This file provides an encoder and decoder for NVGIFv6.
"""
import zlib
import warnings
import binascii
...
class NVGIFv6:
    ...
    def encode(self, image_or_path, out_path, compression=CompressionType.RLE | CompressionType.ZLIB, alpha=False):
        ...
        with open(out_path, "wb") as f:
            ...
            f.write(binascii.crc32(raw).to_bytes(4, "big"))
            f.write(raw)
    ...
    def decode(self, nvg_path, png_path=RETURN_IMAGE):
        ...
        if checksum != binascii.crc32(raw):
            raise OSError("checksum validation failed")
        ...
        out = Image.frombytes(mode, (width, height), bytes(pixels))
